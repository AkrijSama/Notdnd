...
|---|---|---|
| **1.1 chaosling/violet → world.corruption** ✅ DONE | `bestiary.js:299-320` mintChaosling hardcodes `kind:"chaosling"` / `name:"Chaos-"` / `corruption:{palette:"violet"}` / `tags`, called by `worldBook.js:310` (creator door) + `bestiary.js:510` (runtime). Coupled to the ledger's **9-module chaos-vocabulary span**: `essence.js` SIGHT_PHRASES + `chaoslingSpawn.js` + `CHAOS_SKILLS`, and the parity/nature-coherence tests asserting exact Babel strings. | **MET:** `mintChaosling` now reads a resolved `corruption` identity (default `CHAOSLING_CORRUPTION` = byte-identical); babel authors `world.corruption` (byte-identical golden); `NEUTRAL_CORRUPTION` → `kind:"beast"`, no violet, no `corrupted` tag, essence-sight silent at the mint AND player door (`readableEnemiesAt`). `corruption` added to `CARRIED_WORLD_KEYS.object`; both doors green. The 9-module chaos-vocab span (`essence.js` phrase fns, `chaoslingSpawn.js` role/tags) is gated on `sightReadableSkills`/`chaosSkills` → a neutral world reaches NONE of it. `tests/corruption-furniture.test.js` (6/6); 30/30 existing chaos tests unchanged. |
| **2.1 rankLadder → progression** ✅ DONE | `progression.js:92` `RANK_LADDER` is a hardcoded ARRAY; the loader's `carryObject` skips arrays (`held-item-contract.test.js` rankLadder probe locks this). | **MET:** `rankLadder` moved to `CARRIED_WORLD_KEYS.array`; `resolveRankLadder(world)` + `rankForPlayer(player, world)` read `world.rankLadder` else E→DG; babel authors its E→DG ladder (byte-identical); `held-item` probe FLIPPED to RESOLVED; cyberpunk world proves a 5-rung ladder ("Legend", not "A"). Door: the hunter-rank readout in the STATUS WINDOW. |
| **2.2 sheetSpec → status window** ✅ DONE | The STATUS WINDOW render is client-gated `variant==="babel"` (`soloSceneShell.js`); `scene.js` 4-field server weld + `goalDoors.js:179` system-voice weld. | **MET:** server (`scene.js buildPlayerPayload`) emits rank/rankedSkillCount/babelStats/babelSkills gated on `world.sheetSpec` (family "babel" → the six-stat spine); `goalDoors.js` system-voice gated on sheetSpec; client (`characterFromScenePlayer`) opens the diegetic WINDOW on sheetSpec presence + relabels via `sheetSpec.statLabels`. Babel authors a sheetSpec byte-identical; cyberpunk renders its own DECK (BODY/REFLEX/TECH/COOL), babelStats null. `held-item` probe FLIPPED. Door: the character-sheet WINDOW. |
| **2.3 threatLadder → encounters** ✅ DONE | `bestiary.js:217` frozen `THREAT_LADDER`; encounter/spawn selection never reads `world.threatLadder`. | **MET:** `mintStarterEncounter` reads the first COMMON-rarity rung of `world.threatLadder` and keyword-maps it to the starter chassis (`matchChassisKeyword`, exported from bestiary). A hound-drone ladder mints a wolf; a bear-thing ladder a bear; the default "wildlife" rung (no species keyword) → the deterministic pick, so babel (authored placements, never minted) + thin worlds are unchanged. Slot consumer note flipped DEAD→LIVE. Door: the tier-1 encounter at the edge of the kept ground. |
| **2.4 remaining dead slots** (orientationMix, nameBanks, speechConventions, playerSense, suggestionExemplars) ✅ DONE | carried by the loader, zero live run.world readers (the stage-3 "widening" carried them for a FUTURE consumer — the third state the owner now forbids). | **MET (no third state):** all five dead **carries** removed from `CARRIED_WORLD_KEYS`. Dispositions: **nameBanks** → WIRED, its live reader is the COMPILE-time cast/POI name mint (`book.nameBanks`); only the redundant run.world carry was dead → removed, solicitation kept. **orientationMix** → REMOVED carry (its declared consumer, the reputation romance path, never reads it — still an honest manifest DEAD SLOT; not author-solicited, only an override hook). **playerSense / speechConventions / suggestionExemplars** → phantom carries (no author, reader, or solicitation) → REMOVED. `steel-furniture-widening` + `world-key-carry-parity` rewritten to lock the removals. `sightAccent` is `styles.css` — **CLI-1 fenced, untouched (reported, not modified).** |
