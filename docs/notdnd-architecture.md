...
- Rest is a deterministic time/recovery verb: it respects current-location rest metadata, advances server world time, recovers simple resources if present, logs a timeline event, and leaves full 5e rest, spell, camp, survival, and random encounter systems for later.
- Use Item is a deterministic inventory verb: it validates player-held items, applies only predefined item effects, can recover simple resources, reveal predefined notes, consume quantity, and logs timeline events without loot, crafting, equipment, combat, AI calls, or lore invention.
